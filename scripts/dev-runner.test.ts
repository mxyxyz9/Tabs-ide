  ...
  describe("resolveOffset", () => {
    ...
    it.effect("throws for negative port offset", () =>
      Effect.gen(function* () {
        ...
        assert.ok(error.includes("Invalid TABS_PORT_OFFSET"));
      }),
    );
  });

  describe("createDevRunnerEnv", () => {
    it.effect("defaults TABS_HOME to ~/.tabs when not provided", () =>
      Effect.gen(function* () {
        ...
        assert.equal(env.TABS_HOME, resolve(homedir(), ".tabs"));
      }),
    );
    ...
    it.effect("supports explicit typed overrides", () =>
      Effect.gen(function* () {
        ...
        assert.equal(env.TABS_HOME, resolve("/tmp/custom-tabs"));
        assert.equal(env.TABS_PORT, "4222");
        assert.equal(env.VITE_WS_URL, "ws://localhost:4222");
        assert.equal(env.TABS_NO_BROWSER, "1");
        assert.equal(env.TABS_AUTO_BOOTSTRAP_PROJECT_FROM_CWD, "0");
        assert.equal(env.TABS_LOG_WS_EVENTS, "1");
        assert.equal(env.TABS_HOST, "0.0.0.0");
    ...
    it.effect("does not force websocket logging on in dev mode when unset", () =>
      Effect.gen(function* () {
        const env = yield* createDevRunnerEnv({
          mode: "dev",
          baseEnv: {
            TABS_LOG_WS_EVENTS: "keep-me-out",
          },
          serverOffset: 0,
          webOffset: 0,
          tabsHome: undefined,
        ...
        assert.equal(env.TABS_MODE, "web");
        assert.equal(env.TABS_LOG_WS_EVENTS, undefined);
      }),
    );
    ...
    it.effect("forwards explicit websocket logging false without coercing it away", () =>
      Effect.gen(function* () {
        ...
        assert.equal(env.TABS_LOG_WS_EVENTS, "0");
      }),
    );

    it.effect("uses custom tabsHome when provided", () =>
      Effect.gen(function* () {
        const env = yield* createDevRunnerEnv({
          mode: "dev",
          baseEnv: {},
        ...
        assert.equal(env.TABS_HOME, resolve("/tmp/my-tabs"));
      }),
    );
    ...
    it.effect("does not export backend bootstrap env for dev:desktop", () =>
      Effect.gen(function* () {
        ...
        assert.equal(env.TABS_PORT, undefined);
        assert.equal(env.TABS_AUTH_TOKEN, undefined);
        assert.equal(env.TABS_MODE, undefined);
        assert.equal(env.TABS_NO_BROWSER, undefined);
        assert.equal(env.TABS_HOST, undefined);
