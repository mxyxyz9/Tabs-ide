...
- `/apps/desktop`: Electron shell. Spawns a desktop-scoped `tabs` backend process and loads the shared web app.
...
- `/packages/shared`: Shared runtime utilities consumed by both server and web. Uses explicit subpath exports (e.g. `@tabs/shared/git`, `@tabs/shared/DrainableWorker`) — no barrel index.
