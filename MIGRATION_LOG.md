...
- Added the native `ElectronPtyHostStarter` / `PtyHostService` and registered the `localPty` Electron IPC channel. Added the platform external-terminal service on its standard `externalTerminal` channel. Both mirror Code-OSS electron-main construction and remain inside the Tabs-owned native boundary.
- The minimal configuration-service adapter initially lacked `onDidChangeConfiguration`; the PTY host started but restarted after that missing event surfaced. Added the standard no-event implementation and repeated the launch from a clean process.
- Terminal verified end to end: the Tabs control extension executed `workbench.action.terminal.toggleTerminal`, the workbench displayed `Terminal 1, zsh`, the PTY utility process remained live, and a command typed into xterm created `/tmp/tabs-terminal-verified`. The verification file was deleted afterward.
- Native filesystem verified again in the final launch: Quick Open loaded a disposable workspace file, editor input replaced its contents, Save persisted `native` to the host filesystem, and the disposable file was removed afterward.
- Extension behavior verified again: the local extension host started, the Tabs integration extension activated, connected its authenticated control channel, executed theme/file/terminal/debug commands, and built-in JSON and Markdown language-server child processes were observed.
- Debug launch was exercised with a disposable Node launch configuration. Code-OSS recognized and selected `Native debug probe`, but displayed `Install node Extension`; the compiled `tabs-code-main` runtime does not contain a Node debug adapter capable of launching it. The disposable configuration and program were removed. Debugging is therefore **degraded/unverified**, not reported as working.
- Residual scan across `tabs-main` is clean for `server-main.js`, `BrowserSocketFactory`, `RemoteFileSystemProviderClient`, `managed-server`, and `TABS_CODE_OSS_ENTRY`. No source branch on `codeHostConfig.runtime.kind` remains. The `ws://127.0.0.1` construction still present in `main.ts` is the Tabs application backend transport, not Code-OSS embedding or REH.
- Removed stale server bootstrap validation and language from desktop artifact assembly while retaining root `node_modules` packaging required by native main-process services and built-in extensions. Updated user-facing runtime instructions and the architecture document to describe the native-only path.
- Fully working: native workbench load, native Electron IPC connection, extension-host spawn/activation/control, `Schemas.file` read/write persistence, utility worker registration, per-project session ownership, and integrated terminal command execution.
