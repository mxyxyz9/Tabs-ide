...
1. **Finish read-only parity:** add user-controlled PR-list pagination. Provider-backed PR file/diff data and the Code tab are implemented with bounded transport.
...
The Git tool should not be described as PR-parity complete until pagination, inline collaboration, and provider-capability gating are implemented and tested against at least GitHub plus one non-GitHub provider. The current implementation is a broad Git client with a real GitHub/GitLab PR management and read-only code workspace, not yet a complete collaborative PR-review client.
