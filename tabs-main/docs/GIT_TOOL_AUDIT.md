...
| Review interaction       | Comments, approve/request-changes, reviewer/label edits, provider-backed inline threads, selectable diff lines, GFM rendering, plus separate local AI review | Inline annotations, comments, reactions, reviewer selection, Markdown editing | Comments, Markdown, review timeline and code inspection         | Reactions remain                                      |
...
2. **Finish review parity:** reactions; provider-backed inline discussions, selectable old/new diff lines, replies, GFM descriptions/comments, review submission, and reviewer management already work.
...
The Git tool should not be described as full T3 PR parity until reactions, GitHub GraphQL thread resolution, and the remaining provider adapters are implemented and tested. It is now a broad, capability-aware Git client with real GitHub/GitLab PR management, provider-backed inline review collaboration, and selectable diff-line targets—not merely a read-only code workspace.
