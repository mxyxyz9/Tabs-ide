...
4. **Panel interaction coverage — in progress.** Browser tests exercise loading/setup gates,
   destructive discard and force-push confirmations, unsuccessful authentication staying open,
   error-boundary recovery, and an injected remote environment handling an actual diff action. PR
   state transitions still need browser-level coverage.
