...
| Provider/account support | GitHub/GitLab full review adapters; Azure DevOps list/detail/create/core-action adapter; Bitbucket discovery only                                                       | GitHub, GitLab, Azure DevOps, Bitbucket PR providers                          | Deep hosted-PR service and repository inventory                 | Finish Bitbucket and advanced Azure actions           |
...
3. **Typed PR mutation — GitHub, GitLab, and Azure DevOps implemented.** The repository remote selects the provider before list, detail, creation, or mutation. All adapters pass argument arrays directly to their CLI and normalize real provider metadata into the shared model. Azure advertises no diff or comment controls because its current CLI surface cannot fulfill them. Bitbucket and advanced Azure auto-merge/edit actions remain.
...
The Git tool should not be described as full T3 provider parity until Bitbucket and the remaining advanced Azure actions are implemented and tested. GitHub and GitLab have real provider-backed inline collaboration, reactions, resolution, GFM content, and selectable diff-line targets; Azure now has capability-gated list/detail/create and core lifecycle/reviewer actions.
