...
The Git tool should not be described as full T3 provider parity until Azure DevOps and Bitbucket adapters are implemented and tested. GitHub and GitLab now have real provider-backed inline collaboration, reactions, resolution, GFM content, and selectable diff-line targets—not merely a read-only code workspace.
