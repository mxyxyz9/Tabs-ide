...
The Git tool now routes the four T3 providers through real adapters, including provider-aware local/worktree preparation instead of unconditionally invoking GitHub. Full fine-grained parity still requires Bitbucket reviewer lookup/mutation and provider-side search. GitHub/GitLab have reactions and rich review collaboration; Bitbucket has real diffs, inline collaboration, and pull-request text editing; Azure has capability-gated list/detail/create, lifecycle/reviewer/edit actions, and provider-backed auto-complete.
