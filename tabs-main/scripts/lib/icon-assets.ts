...
function createIcoBuffer(icons: ReadonlyArray<{ readonly size: number; readonly png: Buffer }>): Buffer {
...
export function prepareBrowserBrandAssets(clientDir: string, repoRoot: string) {
  return Effect.gen(function* () {
    ...
    for (const fileName of LEGACY_BROWSER_ICON_FILENAMES) {
      yield* fs.remove(path.join(clientDir, fileName), { force: true }).pipe(
        Effect.catch(() => Effect.void),
      );
    }
    yield* fs.copyFile(lightSvgPath, path.join(clientDir, PUBLIC_BRAND_ICON_FILENAMES.iconLightSvg));
    yield* fs.copyFile(darkSvgPath, path.join(clientDir, PUBLIC_BRAND_ICON_FILENAMES.iconDarkSvg));
    yield* renderSvgToIcoFile(lightSvgPath, path.join(clientDir, PUBLIC_BRAND_ICON_FILENAMES.faviconIco));
