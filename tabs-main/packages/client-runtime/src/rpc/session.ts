import { type ServerConfig, WS_METHODS } from "@tabs/contracts";
...
export const make = Effect.gen(function* () {
  ...
  const connect = Effect.fnUntraced(function* (connection: PreparedConnection) {
    ...
    const probe = initialConfig.pipe(
      Effect.flatMap(() =>
        client[WS_METHODS.serverGetConfig]({}).pipe(Effect.mapError(mapSessionRpcError)),
      ),
      Effect.asVoid,
