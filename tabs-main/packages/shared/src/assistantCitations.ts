...
const ALT_CITATION_PROTOCOL = "tabs-citation:";
const ALT_CITATION_HREF_PREFIX = `${ALT_CITATION_PROTOCOL}//v1/`;
...
const CITATION_LINK = new RegExp(
  String.raw`\[Assistant quote\]\(((?:tabs-citation:|t3-citation:)\/\/v1\/[^\s)]{1,${MAX_CITATION_HREF_LENGTH - CITATION_HREF_PREFIX.length}})\)`,
  "g",
);
...
export function parseAssistantCitationHref(href: string): AssistantCitation | null {
  const isModern = href.startsWith(CITATION_HREF_PREFIX);
  const isAlt = href.startsWith(ALT_CITATION_HREF_PREFIX);
  if ((!isModern && !isAlt) || href.length > MAX_CITATION_HREF_LENGTH) {
    return null;
  }
  try {
    const url = new URL(href);
    const parts = url.pathname.slice(1).split("/");
    const expectedProtocol = isModern ? CITATION_PROTOCOL : ALT_CITATION_PROTOCOL;
    if (
