...
it.effect("accepts provider-scoped model options in thread.turn.start", () =>
  Effect.gen(function* () {
    ...
    assert.strictEqual(parsed.modelSelection?.instanceId, "codex");
    const turnOptions = parsed.modelSelection?.options ?? [];
    assert.strictEqual(turnOptions.find((o) => o.id === "reasoningEffort")?.value, "high");
    assert.strictEqual(turnOptions.find((o) => o.id === "fastMode")?.value, true);
  }),
);
