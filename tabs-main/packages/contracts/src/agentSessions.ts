...
/**
 * Scan options for agent session discovery.
 */
 ...
 * Tabs project. `alreadyImported` marks candidates that already have an
...
export class AgentSessionScanError extends Schema.TaggedErrorClass<AgentSessionScanError>()(
  ...
  {
    operation: Schema.Literals(["read-settings", "read-projects", "read-transcripts"]),
    cause: Schema.Defect(),
  },
) {
  override get message(): string {
