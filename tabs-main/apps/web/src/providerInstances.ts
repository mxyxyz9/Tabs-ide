...
/**
 * Format a driver kind slug into a title-case label. Used as a fallback when
 * `PROVIDER_DISPLAY_NAMES` doesn't have a mapping for the given kind.
 */
function formatDriverKindLabel(driverKind: ProviderDriverKind): string {
  return driverKind
    .replace(/[_-]+/g, " ")
    .replace(/([a-z])([A-Z])/g, "$1 $2")
    .split(" ")
...
function driverKindLabel(driverKind: ProviderDriverKind): string {
  return PROVIDER_DISPLAY_NAMES[driverKind] ?? formatDriverKindLabel(driverKind);
}
