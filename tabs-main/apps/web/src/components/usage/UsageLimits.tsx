...
export function useResetCredit(
  ...
  const redeem = async () => {
    ...
    setStatus(null);
    try {
      const result = await ensureNativeApi().server.consumeResetCredit(input);
      setBusy(false);
  ...
  const presentations = useMemo(() => {
    const map = new Map<
      EnvironmentId,
      {
        ...
        readonly serverConfig: typeof serverConfig;
      }
    >();

    const localId = "local" as EnvironmentId;
    if (
      selectedEnvironmentIds === undefined ||
      selectedEnvironmentIds === null ||
      selectedEnvironmentIds.has(localId)
    ) {
      map.set(localId, {
    ...
    return map;
  }, [serverConfig, selectedEnvironmentIds]);

  return <UsageLimitsPooled presentations={presentations} now={now} />;
}
