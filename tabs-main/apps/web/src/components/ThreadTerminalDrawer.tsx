...
export default function ThreadTerminalDrawer({
  ...
  return (
    <aside
      className={
        variant === "embedded"
          ? "thread-terminal-drawer relative flex h-full min-h-0 min-w-0 flex-1 flex-col overflow-hidden bg-background"
          : "thread-terminal-drawer relative flex min-w-0 shrink-0 flex-col overflow-hidden border-t border-border/80 bg-background"
      }
      style={variant === "embedded" ? undefined : { height: `${drawerHeight}px` }}
    >
      {variant === "drawer" ? (
        <div
      ...
      {showControls && !hasTerminalSidebar && (
