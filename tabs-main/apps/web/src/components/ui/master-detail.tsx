...
export function MasterDetailSidebar({ children, className }: { children: ReactNode; className?: string }) {
  return (
    <div className={cn("flex w-56 shrink-0 flex-col gap-3 border-r border-border/70 pr-6", className)}>
      {children}
    </div>
  );
}
  ...
  return (
    <div ref={ref} className={cn("group flex items-center gap-1", className)} {...props}>
      ...
      >
        ...
        {isUnsaved ? (
          <span className="absolute right-2 top-1/2 -translate-y-1/2 w-1.5 h-1.5 rounded-full bg-blue-500" title="Unsaved changes" />
        ) : null}
      </Button>
      {dragHandleProps ? (
