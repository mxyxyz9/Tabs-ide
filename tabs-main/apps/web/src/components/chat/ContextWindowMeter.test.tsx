import { EventId, TurnId } from "@tabs/contracts";
...
describe("ContextWindowMeter", () => {
  ...
  it("closes an informational hover popover without delay when compact is not offered", () => {
  ...
  it("explains why the compact action is disabled", () => {
    const markup = renderToStaticMarkup(
      <ContextWindowMeter
        ...
        onCompact={() => {}}
        compactDisabled
        compactDisabledReason="Start a conversation before compacting"
      />,
    );

    expect(markup).toContain("disabled");
    expect(markup).toContain("Start a conversation before compacting");
  });
});
