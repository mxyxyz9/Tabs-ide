...
import {
  ...
  type AssistantCitationListHandle,
  type AssistantCitationTarget,
} from "./AssistantCitationSource";

const mocks = { resolveRange: vi.fn(), toast: vi.fn() };
...
function createSource({ reducedMotion = false } = {}) {
  ...
  const list: AssistantCitationListHandle = {
    getScrollableNode: () => scrollNode as unknown as HTMLElement,
    getState: () => ({
  ...
  return {
    ...
    highlight: () =>
      highlights.get("tabs-assistant-citation")?.values().next().value ??
      highlights.get("t3-assistant-citation")?.values().next().value,
    commentHighlight: () =>
      highlights.get("tabs-assistant-citation-comment") ??
      highlights.get("t3-assistant-citation-comment"),
    cleanup: () => {
...
describe("assistant citation source lifecycle", () => {
  ...
  it("does not erase another highlight when its source unmounts", async () => {
    ...
    source.highlights.set("tabs-assistant-citation", replacement);
    source.highlights.set("t3-assistant-citation", replacement);
    source.cleanup();
    expect(source.highlights.get("tabs-assistant-citation")).toBe(replacement);
  });
...
describe("assistant citation comment source lifecycle", () => {
  it("keeps the exact live range highlighted until cleanup without changing selection or navigation", () => {
    ...
    source.highlights.set("tabs-assistant-citation", navigationHighlight);
    ...
    expect(source.highlights.get("tabs-assistant-citation")).toBe(navigationHighlight);
  ...
  it("does not delete a replacement highlight when an older editor closes", () => {
    ...
    source.highlights.set("tabs-assistant-citation-comment", replacement);
