...
export interface ChatComposerProps {
  ...
  isLoading?: boolean;
  error?: string | null;
  pendingApprovals?: { requestId: ApprovalRequestId; isResponding: boolean; onRespondToApproval: (requestId: ApprovalRequestId, decision: ProviderApprovalDecision) => Promise<void> }[];
  ...
  onApprovalAction?: (requestId: ApprovalRequestId, decision: ProviderApprovalDecision) => Promise<void>;
...
export function ChatComposer({
  ...
  const handleProviderSelect = useCallback((newProvider: ProviderKind, model: string) => {
    ...
    <div className={cn("flex flex-col gap-4 p-4 border-t bg-background", className)}>
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          <select
            value={provider}
            onChange={(e) => handleProviderSelect(e.target.value as ProviderKind, modelSelection?.model || "gpt-4")}
            disabled={isDisabled}
