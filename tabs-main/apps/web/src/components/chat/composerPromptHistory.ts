...
export const PLAN_IMPLEMENTATION_PROMPT_PREFIX = "Implement the following proposed plan:\n\n";
...
function stripTrailingPreviewAnnotation(prompt: string): string {
  const match = TRAILING_PREVIEW_ANNOTATION_BLOCK_PATTERN.exec(prompt);
  if (!match) return prompt;
  return prompt.slice(0, match.index).replace(/\n+$/, "");
}

function stripTrailingElementContexts(prompt: string): string {
...
export function recallableComposerPrompt(messageText: string): string {
  ...
  while (prompt.length > 0) {
    ...
    const withoutPreview = stripTrailingPreviewAnnotation(prompt);
    if (withoutPreview !== prompt) {
      prompt = withoutPreview;
      continue;
    }
    const withoutElement = stripTrailingElementContexts(prompt);
    if (withoutElement !== prompt) {
      prompt = withoutElement;
      continue;
    }
