import type { GitRunStackedActionResult, GitStackedAction, GitStatusResult } from "@tabs/contracts";
...
export function buildMenuItems(
  ...
  const canCommit = !isBusy && hasChanges && !hasConflicts && !hasOperation;
  const canPush =
    !isBusy &&
    hasBranch &&
    !hasChanges &&
...
export function resolveQuickAction(
  ...
  if (hasOperation || hasConflicts) {
    return {
      label: "Resolve conflicts",
      disabled: true,
      kind: "show_hint",
      hint: hasOperation
        ? `A ${gitStatus.operation?.kind ?? "git"} is in progress. Resolve conflicts or continue/abort it from the Git tab first.`
        : "Resolve conflicted files before using commit, push, or PR actions.",
    };
  }
...
export { resolveAutoFeatureBranchName } from "@tabs/shared/git";
