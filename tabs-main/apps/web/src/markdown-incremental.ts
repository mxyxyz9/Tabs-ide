...
function hasDefinitions(node: MarkdownAstNode): boolean {
  return (
    node.type === "definition" ||
    node.type === "footnoteDefinition" ||
    (Array.isArray(node.children) && node.children.some((child) => hasDefinitions(child)))
  );
}

function shiftPositions(node: MarkdownAstNode, offset: number, lines: number): void {
  if (node.position) {
    for (const point of [node.position.start, node.position.end]) {
      if (point && point.offset !== undefined) point.offset += offset;
      if (point) point.line += lines;
    }
  }
...
export function createIncrementalMarkdownParser(
  parse: (source: string, file?: unknown) => MarkdownAstNode,
): (source: string, file?: unknown) => MarkdownAstNode {
  ...
  return (source, file) => {
    ...
    for (let index = children.length - 1; index >= 0; index--) {
      ...
      cached = {
        ...
        offset,
        line: (node.position?.end?.line ?? 0) + 2,
 ...
 */
export function createIncrementalMarkdownPlugin(): any {
  let parser: ((source: string, file?: unknown) => MarkdownAstNode) | undefined;
  return function (this: { parser?: (source: string, file?: unknown) => MarkdownAstNode }) {
