...
function readHostSystemLocale(): string | null {
  ...
  return (window as any).desktopBridge?.getSystemLocale?.() ?? null;
}
...
export function formatRelativeTime(isoDate: string): RelativeTimeParts {
  const date = parseTimestampDate(isoDate);
  if (!date) return { value: "just now", suffix: null };
