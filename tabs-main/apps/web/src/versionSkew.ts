...
import { compareSemverVersions, parseSemver } from "@tabs/shared/semver";
...
export const VERSION_MISMATCH_DISMISSALS_STORAGE_KEY = "tabs:version-mismatch-dismissals:v1";
...
/**
 * The skew a user can act on: the connected server runs an older Tabs version than
 * this client, so the server or runtime is the side that needs updating.
 */
export function resolveVersionMismatch(
  ...
  return {
    ...
    hint: "Version mismatch. Try syncing the client and server to the same Tabs version.",
  };
}
...
export function resetVersionMismatchDismissals(): void {
  writeVersionMismatchDismissals({ keys: [] });
}

export function manualServerUpdateCommand(targetVersion: string): string {
  return `bun run --cwd apps/server build`;
}
