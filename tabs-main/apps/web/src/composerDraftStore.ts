...
import { getDefaultModel, normalizeModelSlug } from "@tabs/shared/model";
...
const PersistedComposerThreadDraftState = Schema.Struct({
  ...
  modelSelectionByProvider: Schema.optionalKey(
    Schema.Record(ProviderInstanceId, Schema.optional(ModelSelection)),
  ),
...
interface ComposerDraftStoreState {
  ...
  addPreviewAnnotation: (threadId: ThreadId, annotation: PreviewAnnotationPayload) => void;
  setPreviewAnnotations: (threadId: ThreadId, annotations: PreviewAnnotationPayload[]) => void;
  removePreviewAnnotation: (threadId: ThreadId, annotationId: string) => void;
  addReviewComment: (threadId: ThreadId, comment: ReviewCommentContext) => void;
  setReviewComments: (threadId: ThreadId, comments: ReadonlyArray<ReviewCommentContext>) => void;
  removeReviewComment: (threadId: ThreadId, commentId: string) => void;
  setModelSelection: (
    threadId: ThreadId,
  ...
  setProviderModelOptions: (
    ...
    options?: {
      ...
      /** Current effective model, used when the draft has no stored selection
       *  yet (e.g. the model lives on the thread) so options don't reset it. */
      model?: string | null;
    },
  ) => void;
...
/**
 * Per-provider composer option selections, keyed by provider/instance slug.
 * Carries the canonical `ProviderOptionSelection[]` wire form so the composer
 * works generically for any driver (codex, claude, cursor, grok, opencode, …).
 */
export type ComposerModelOptionsByProvider = Partial<
  Record<string, ReadonlyArray<ProviderOptionSelection>>
>;

export interface EffectiveComposerModelState {
  selectedModel: ModelSlug;
  modelOptions: ComposerModelOptionsByProvider | null;
}

function providerModelOptionsFromSelection(
  modelSelection: ModelSelection | null | undefined,
): ComposerModelOptionsByProvider | null {
  if (!modelSelection?.options) {
    return null;
  }
  return { [modelSelection.instanceId]: modelSelection.options };
}
...
function shouldPersistDraftThreadState(
  draftThread: DraftThreadState,
  persistedDraft: PersistedComposerThreadDraftState | undefined,
): boolean {
  if (persistedDraft) {
    return true;
  }
  ...
  return (
    ...
    draftThread.runtimeMode !== DEFAULT_RUNTIME_MODE ||
    draftThread.interactionMode !== DEFAULT_INTERACTION_MODE
  );
}
...
function normalizeModelSelection(
  ...
  const candidate = value && typeof value === "object" ? (value as Record<string, unknown>) : null;
  // Routing keys on the instance id; accept any non-empty provider slug so
  // selections for non-builtin drivers (cursor/grok/opencode) are preserved
  // rather than silently dropped.
  const providerRaw = candidate?.instanceId ?? candidate?.provider ?? legacy?.provider;
  const provider = typeof providerRaw === "string" && providerRaw.length > 0 ? providerRaw : null;
  if (provider === null) {
    return null;
  }
  ...
  // Wire selections already carry options in the canonical `{ id, value }[]`
  // form; only legacy typed-option input needs bridging via the per-provider
  // normalizers (which are codex/claude-specific).
  let optionSelections: ModelSelection["options"] | undefined;
  if (Array.isArray(candidate?.options)) {
    optionSelections = candidate.options as ModelSelection["options"];
  } else if (provider === "codex" || provider === "claudeAgent") {
    const modelOptions = normalizeProviderModelOptions(
      legacy?.modelOptions,
      provider,
      provider === "codex" ? legacy?.legacyCodex : undefined,
    );
    optionSelections = typedOptionsToSelections(
      provider === "codex" ? modelOptions?.codex : modelOptions?.claudeAgent,
    );
  }
  return makeAppModelSelection(provider, model, optionSelections);
}
...
function legacySyncModelSelectionOptions(
  ...
  const provider = modelSelection.instanceId;
  const options = modelOptions?.[provider as keyof ProviderModelOptions];
  return makeAppModelSelection(provider, modelSelection.model, typedOptionsToSelections(options));
}
...
function legacyMergeModelSelectionIntoProviderModelOptions(
  ...
  return legacyReplaceProviderModelOptions(
    ...
    modelSelection.instanceId as ProviderKind,
    selectionsToTypedOptions(modelSelection.options) as ProviderModelOptions[ProviderKind],
  );
}
...
function legacyToModelSelectionByProvider(
  ...
  if (modelOptions) {
    for (const provider of ["codex", "claudeAgent"] as const) {
      ...
      if (options && Object.keys(options).length > 0) {
        result[provider] = makeAppModelSelection(
          provider,
  ...
  if (modelSelection) {
    result[modelSelection.instanceId as ProviderKind] = modelSelection;
  }
  return result;
}
...
function normalizePersistedDraftsByThreadId(
  ...
  for (const [threadId, draftValue] of Object.entries(rawDraftMap as Record<string, unknown>)) {
    ...
    const previewAnnotations = Array.isArray(draftCandidate.previewAnnotations)
      ? draftCandidate.previewAnnotations.flatMap((entry) => {
          try {
            return [Schema.decodeUnknownSync(PreviewAnnotationPayloadSchema)(entry)];
          } catch {
            return [];
          }
        })
    ...
    const reviewComments = Array.isArray(draftCandidate.reviewComments)
      ? draftCandidate.reviewComments.flatMap((entry) => {
          try {
            return [Schema.decodeUnknownSync(ReviewCommentContextSchema)(entry)];
          } catch {
            return [];
          }
        })
    ...
    if (
      ...
      activeProvider = (modelSelection?.instanceId ?? null) as ProviderKind | null;
    }
    ...
    } as unknown as DeepMutable<PersistedComposerThreadDraftState>;
  }
...
function partializeComposerDraftStoreState(
  ...
  const persistedDraftThreadsByThreadId: DeepMutable<
    PersistedComposerDraftStoreState["draftThreadsByThreadId"]
  > = {};
  for (const [threadId, draftThread] of Object.entries(state.draftThreadsByThreadId)) {
    ...
    const persistedDraft = persistedDraftsByThreadId[threadId as ThreadId];
    if (!shouldPersistDraftThreadState(draftThread, persistedDraft)) {
      continue;
    }
  ...
  const persistedProjectDraftThreadIdByProjectId: DeepMutable<
    PersistedComposerDraftStoreState["projectDraftThreadIdByProjectId"]
  > = {};
  for (const [projectId, threadId] of Object.entries(state.projectDraftThreadIdByProjectId)) {
    if (
      typeof projectId !== "string" ||
      projectId.length === 0 ||
      typeof threadId !== "string" ||
    ...
    persistedProjectDraftThreadIdByProjectId[projectId as ProjectId] = threadId as ThreadId;
  }

  return {
    draftsByThreadId: persistedDraftsByThreadId,
    draftThreadsByThreadId: persistedDraftThreadsByThreadId,
    projectDraftThreadIdByProjectId: persistedProjectDraftThreadIdByProjectId,
...
function normalizeCurrentPersistedComposerDraftStoreState(
  ...
  const normalizedDraftsByThreadId = normalizePersistedDraftsByThreadId(
  ...
  const filteredDraftThreadsByThreadId: DeepMutable<
    PersistedComposerDraftStoreState["draftThreadsByThreadId"]
  > = {};
  for (const [threadId, draftThread] of Object.entries(draftThreadsByThreadId)) {
    if (
      typeof threadId !== "string" ||
      !shouldPersistDraftThreadState(draftThread, normalizedDraftsByThreadId[threadId as ThreadId])
    ) {
      continue;
    }
    filteredDraftThreadsByThreadId[threadId as ThreadId] = draftThread;
  }
  const filteredProjectDraftThreadIdByProjectId: DeepMutable<
    PersistedComposerDraftStoreState["projectDraftThreadIdByProjectId"]
  > = {};
  for (const [projectId, threadId] of Object.entries(projectDraftThreadIdByProjectId)) {
    if (
      typeof projectId !== "string" ||
      typeof threadId !== "string" ||
      !filteredDraftThreadsByThreadId[threadId as ThreadId]
    ) {
      continue;
    }
    filteredProjectDraftThreadIdByProjectId[projectId as ProjectId] = threadId as ThreadId;
  }
  ...
  return {
    draftsByThreadId: normalizedDraftsByThreadId,
    draftThreadsByThreadId: filteredDraftThreadsByThreadId,
    projectDraftThreadIdByProjectId: filteredProjectDraftThreadIdByProjectId,
    stickyModelSelectionByProvider,
    stickyActiveProvider,
  };
}
    ...
    (set, get) => ({
      ...
      setStickyModelSelection: (modelSelection) => {
        ...
        set((state) => {
          ...
          const stickyProvider = normalized.instanceId as ProviderKind;
          const nextMap: Partial<Record<ProviderKind, ModelSelection>> = {
            ...state.stickyModelSelectionByProvider,
            [stickyProvider]: normalized,
          };
          if (Equal.equals(state.stickyModelSelectionByProvider, nextMap)) {
            return state.stickyActiveProvider === stickyProvider
              ? state
              : { stickyActiveProvider: stickyProvider };
          }
          return {
      ...
      addPreviewAnnotation: (threadId, annotation) => {
        if (threadId.length === 0) return;
        set((state) => {
          const existing = state.draftsByThreadId[threadId] ?? createEmptyThreadDraft();
          const previewAnnotations = existing.previewAnnotations.filter(
            (entry) => entry.id !== annotation.id,
          );
          return {
            draftsByThreadId: {
              ...state.draftsByThreadId,
              [threadId]: { ...existing, previewAnnotations: [...previewAnnotations, annotation] },
            },
          };
        });
      },
      setPreviewAnnotations: (threadId, previewAnnotations) => {
        if (threadId.length === 0) return;
        set((state) => {
          const existing = state.draftsByThreadId[threadId] ?? createEmptyThreadDraft();
          return {
            draftsByThreadId: {
              ...state.draftsByThreadId,
              [threadId]: { ...existing, previewAnnotations: [...previewAnnotations] },
            },
          };
        });
      },
      removePreviewAnnotation: (threadId, annotationId) => {
        if (threadId.length === 0 || annotationId.length === 0) return;
        set((state) => {
          const current = state.draftsByThreadId[threadId];
      ...
      addReviewComment: (threadId, comment) => {
        if (threadId.length === 0) return;
        set((state) => {
          const existing = state.draftsByThreadId[threadId] ?? createEmptyThreadDraft();
          ...
          return {
            draftsByThreadId: {
              ...state.draftsByThreadId,
              [threadId]: { ...existing, reviewComments: [...reviewComments, comment] },
            },
          };
        });
      },
      setReviewComments: (threadId, comments) => {
        if (threadId.length === 0) return;
        set((state) => {
          const existing = state.draftsByThreadId[threadId] ?? createEmptyThreadDraft();
          return {
            draftsByThreadId: {
              ...state.draftsByThreadId,
              [threadId]: { ...existing, reviewComments: [...comments] },
            },
          };
        });
      },
      removeReviewComment: (threadId, commentId) => {
        if (threadId.length === 0 || commentId.length === 0) return;
        set((state) => {
          const current = state.draftsByThreadId[threadId];
          if (!current) return state;
      ...
      setModelSelection: (threadId, modelSelection) => {
        ...
        set((state) => {
          ...
          const nextMap = { ...base.modelSelectionByProvider };
          if (normalized) {
            const providerKey = normalized.instanceId as ProviderKind;
            const current = nextMap[providerKey];
            if (normalized.options !== undefined) {
              // Explicit options provided → use them
              nextMap[providerKey] = normalized;
            } else {
              // No options in selection → preserve existing options, update provider+model
              nextMap[providerKey] = makeAppModelSelection(
          ...
          const nextActiveProvider =
            (normalized?.instanceId as ProviderKind | undefined) ?? base.activeProvider;
          if (
            Equal.equals(base.modelSelectionByProvider, nextMap) &&
      ...
      setModelOptions: (threadId, modelOptions) => {
        ...
        set((state) => {
          ...
          for (const provider of ["codex", "claudeAgent"] as const) {
            ...
            if (opts) {
              nextMap[provider] = makeAppModelSelection(
                provider,
                current?.model ?? getDefaultModel(provider),
                typedOptionsToSelections(opts),
              );
      ...
      setProviderModelOptions: (threadId, provider, nextProviderOptions, options) => {
        if (threadId.length === 0 || provider.length === 0) {
          return;
        }
        // Options flow as the canonical `ProviderOptionSelection[]` wire form for
        // any provider/driver — store them verbatim.
        ...
        set((state) => {
          ...
          if (providerOpts) {
            nextMap[normalizedProvider] = makeAppModelSelection(
              normalizedProvider,
              resolvedModel,
              providerOpts,
            );
          ...
          if (options?.persistSticky === true) {
            ...
            const stickyBase =
              ...
              makeAppModelSelection(normalizedProvider, resolvedModel);
            if (providerOpts) {
              nextStickyMap[normalizedProvider] = makeAppModelSelection(
                normalizedProvider,
                stickyBase.model,
                providerOpts,
              );
      ...
      addFiles: (threadId, files) => {
        ...
        set((state) => {
          ...
          return {
            draftsByThreadId: {
              ...state.draftsByThreadId,
              [threadId]: { ...existing, files: [...existing.files, ...accepted] },
            },
          };
        });
      },
