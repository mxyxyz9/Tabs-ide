...
describe("incremental code highlighting", () => {
  it.each(Object.entries(samples))(
    "matches full AST at every streaming prefix in %s",
    async (language, code) => {
      ...
      for (const theme of ["pierre-dark", "pierre-light"] as const) {
        ...
        for (let end = 0; end <= code.length; end++) {
          ...
          expect(highlight(text), `${theme}, prefix ${end}`).toEqual(
            highlighter.codeToHast(text, { lang: language, theme }),
          );
        }
      }
    },
  ...
  it("resets after edits and truncation, including edits to a completed line", async () => {
    ...
    for (const text of inputs) {
      expect(highlight(text)).toEqual(
        highlighter.codeToHast(text, { lang: "typescript", theme: "pierre-dark" }),
      );
    }
  });
  ...
  it.each(["text", "plaintext", "plain", "txt", "ansi"])(
    ...
    async (language) => {
      ...
      for (const text of ["plain\ntext", "\u001b[31mred\ncontinued", "\n"]) {
        expect(highlight(text)).toEqual(
          highlighter.codeToHast(text, { lang: language, theme: "pierre-dark" }),
        );
      }
    },
  );
