import type { EnvironmentId, ScopedProjectRef } from "@tabs/contracts";
import {
  buildProjectGroups,
  type ProjectGroupingSettings,
} from "@tabs/client-runtime/state/project-grouping";
...
export function buildPhysicalToLogicalProjectKeyMap(input: {
  ...
  const runtimeProjects = input.projects.map((p) => ({
    ...p,
    title: p.name,
    workspaceRoot: p.cwd,
    environmentId: p.environmentId ?? input.primaryEnvironmentId ?? ("local" as EnvironmentId),
