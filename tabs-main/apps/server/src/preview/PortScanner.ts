 ...
 * stable line-prefixed field format).
 *
 * Windows / lsof missing: checks a curated list of common dev ports through
 * the shared Net service, or queries PowerShell Get-NetTCPConnection.
 *
...
import { isLoopbackHost, LSOF_LOCAL_HOST_TOKENS } from "@tabs/shared/preview";
...
export class PortDiscoveryProcessRunner extends Context.Service<
  PortDiscoveryProcessRunner,
  ProcessRunnerShape
>()("tabs/preview/PortScanner/ProcessRunner") {}

export const defaultProcessRunnerLive = Layer.succeed(PortDiscoveryProcessRunner, {
  run: (input) =>
    Effect.promise(async () => {
      const timeoutMs =
        input.timeout !== undefined
          ? Duration.toMillis(Duration.fromInputUnsafe(input.timeout))
          : undefined;
...
>()("tabs/preview/PortScanner/PortDiscovery") {}
...
export const parsePortFromLsofName = (name: string): number | null => {
...
export const make = Effect.gen(function* PortDiscoveryMake() {
  ...
  const processRunner = yield* PortDiscoveryProcessRunner;
...
export const layer = Layer.effect(PortDiscovery, make).pipe(
  Layer.provide(defaultProcessRunnerLive),
