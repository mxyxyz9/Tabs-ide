...
import type { ServerProviderUsageLine, ServerProviderUsageLimit } from "@tabs/contracts";

const getGrokApiKeyEnv = (env: NodeJS.ProcessEnv) => env.XAI_API_KEY ?? env.GROK_API_KEY;
import {
...
function grokAuthPaths(ctx: ProviderUsageContext): string[] {
  const paths: string[] = [];
  const push = (val: string) => {
    if (!paths.includes(val)) paths.push(val);
  };
  const grokHome = ctx.env.GROK_HOME?.trim();
  if (grokHome) push(nodePath.join(grokHome, "auth.json"));
  const xdg = ctx.env.XDG_CONFIG_HOME?.trim();
  if (xdg) push(nodePath.join(xdg, "grok", "auth.json"));
  push(nodePath.join(ctx.homeDir, ".grok", "auth.json"));
  push(nodePath.join(ctx.homeDir, ".config", "grok", "auth.json"));
  return paths;
}
...
async function resolveGrokSession(ctx: ProviderUsageContext): Promise<GrokSession | null> {
  for (const path of grokAuthPaths(ctx)) {
