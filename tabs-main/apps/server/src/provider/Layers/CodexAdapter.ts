...
function readPayload<A>(
  ...
  // `Schema.is` requires a schema with `DecodingServices: never`; the generic
  // `Schema.Schema<A>` doesn't surface that, so cast to the parameter type and
  // narrow the validated payload back to `A`.
  const isPayload = Schema.is(schema as Parameters<typeof Schema.is>[0]);
  return isPayload(payload) ? (payload as A) : undefined;
}
