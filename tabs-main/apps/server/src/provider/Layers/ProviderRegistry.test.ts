...
import { checkCodexProviderStatus, type CodexAppServerProviderSnapshot } from "./CodexProvider";
import { checkClaudeProviderStatus } from "./ClaudeProvider";
...
import { ProviderInstanceRegistryHydrationLive } from "./ProviderInstanceRegistryHydration";
import {
...
import { readProviderStatusCache, resolveProviderStatusCachePath } from "../providerStatusCache";
    ...
    describe("ProviderRegistryLive", { timeout: 20_000 }, () => {
        ...
        () =>
          Effect.gen(function* () {
            ...
            yield* Effect.gen(function* () {
              ...
              assert.strictEqual(cursorProvider?.message, "Cursor is disabled in Tabs settings.");
      ...
      it.effect("skips codex probes entirely when the provider is disabled", () =>
        Effect.gen(function* () {
          ...
          assert.strictEqual(status.installed, false);
          assert.strictEqual(status.message, "Codex is disabled in Tabs settings.");
        }),
      );
    });
