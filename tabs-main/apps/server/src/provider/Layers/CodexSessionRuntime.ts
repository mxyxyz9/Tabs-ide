...
import { buildCodexInitializeParams } from "./CodexProvider";
...
function readRouteFields(notification: CodexServerNotification): {
  ...
  switch (notification.method) {
    ...
    case "turn/completed":
      return {
        turnId: notification.params.turn.id as TurnId,
        itemId: undefined,
      };
    case "error":
      return {
        turnId: notification.params.turnId as TurnId,
        itemId: undefined,
      };
    case "turn/diff/updated":
    case "turn/plan/updated":
    ...
    case "item/completed":
      return {
        turnId: notification.params.turnId as TurnId,
        itemId: notification.params.item.id as ProviderItemId,
      };
    case "item/agentMessage/delta":
    case "item/plan/delta":
    case "item/commandExecution/outputDelta":
    ...
    case "item/reasoning/textDelta":
      return {
        turnId: notification.params.turnId as TurnId,
        itemId: notification.params.itemId as ProviderItemId,
      };
    default:
      return {
        turnId: undefined,
...
> =>
  Effect.gen(function* () {
    ...
    const child = yield* spawner
      .spawn(
        ChildProcess.make(options.binaryPath, ["app-server", ...(options.launchArgs ?? [])], {
          cwd: options.cwd,
          env,
        ...
        Effect.mapError(
          (cause) =>
            new CodexErrors.CodexAppServerSpawnError({
              command: `${options.binaryPath} app-server ${(options.launchArgs ?? []).join(" ")}`.trim(),
              cause,
            }),
        ),
      );
    ...
    yield* client.handleServerNotification("turn/started", (payload) =>
      currentSessionProviderThreadId.pipe(
        Effect.flatMap((providerThreadId) => {
          ...
          return updateSession(sessionRef, {
            status: "running",
            activeTurnId: payload.turn.id as TurnId,
          });
        }),
      ),
    );
    ...
    yield* client.handleServerRequest("item/commandExecution/requestApproval", (payload) =>
      Effect.gen(function* () {
        ...
        const itemId = payload.itemId as ProviderItemId;
    ...
    return {
      ...
      sendTurn: (input) =>
        Effect.gen(function* () {
          ...
          const turnId = response.turn.id as TurnId;
          yield* updateSession(sessionRef, {
            status: "running",
            activeTurnId: turnId,
            ...(normalizedModel ? { model: normalizedModel } : {}),
