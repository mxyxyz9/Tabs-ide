...
export interface OpenCodeAdapterLiveOptions {
  ...
  /** Raw event source tag emitted in `raw.source`. Defaults to `"opencode.sdk.event"`. */
  readonly runtimeEventSource?: "opencode.sdk.event" | "kilo.sdk.event";
  /** Prefix for turn IDs generated by this adapter. Defaults to `"opencode-turn"`. */
...
export function makeOpenCodeAdapter(
  ...
  return Effect.gen(function* () {
    ...
    const boundRuntimeEventSource = options?.runtimeEventSource ?? "opencode.sdk.event";
    const boundTurnIdPrefix = options?.turnIdPrefix ?? "opencode-turn";
    ...
    // Provider-bound error helpers — capture boundProvider so events from Kilo
    // sessions are never mis-tagged as coming from OpenCode.
      ...
      Effect.all({
        ...
        Effect.map(({ eventId, createdAt }) => ({
          ...
          ...(input.raw !== undefined
            ? {
                raw: {
                  source: boundRuntimeEventSource,
                  payload: input.raw,
                },
              }
            : {}),
    ...
    const sendTurn: OpenCodeAdapterShape["sendTurn"] = Effect.fn("sendTurn")(function* (input) {
      ...
      const turnId = `${boundTurnIdPrefix}-${yield* randomUUIDv4}` as TurnId;
