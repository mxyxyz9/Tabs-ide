...
import type { ServerProviderSkill } from "@tabs/contracts";
 ...
 * `os.path.expanduser` in the launch environment: `USERPROFILE`, then
 * `HOMEDRIVE` + `HOMEPATH`, on Windows and `HOME` elsewhere. Values are
 * used verbatim; a path may contain spaces.
 */
 ...
 * skills. The agent resolves both under `GEMINI_HOME`, which points at a
