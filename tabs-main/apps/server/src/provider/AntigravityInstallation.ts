...
import { ProviderDriverKind, type ProviderInstallState } from "@tabs/contracts";
import {
...
>()("tabs/provider/AntigravityInstallation") {
...
export const makeAntigravityInstallation = Effect.fn("AntigravityInstallation.make")(function* (
    ...
    Effect.fn("AntigravityInstallation.validate")(
      function* (executable: AntigravityExecutable, expectedVersion: string) {
        ...
        const runtime = yield* makeAntigravityAcpRuntime({
          ...
          clientInfo: { name: "tabs", version: "0.0.0" },
          antigravitySettings: { binaryPath: executable.executablePath },
          environment,
          profileDirectory,
        });
  ...
  const runValidation = (executable: AntigravityExecutable, expectedVersion: string) =>
    Effect.acquireUseRelease(
      Effect.forkScoped(validate(executable, expectedVersion).pipe(Effect.scoped)),
      (fiber) => Fiber.join(fiber).pipe(Effect.timeout(VALIDATION_TIMEOUT)),
  ...
  const install = Effect.fn("AntigravityInstallation.install")(
    function* (asset: AntigravityReleaseAsset) {
      ...
      if (yield* fs.exists(destination)) {
        ...
        yield* runValidation(existing, asset.version);
        yield* activate();
        return;
      }
          ...
          completedRelease(asset.sha256).pipe(
            Effect.flatMap((existing) =>
              existing.version === asset.version
                ? runValidation(existing, asset.version)
                : Effect.fail(
                    installationError(
                      "activate",
  ...
  const start = gate
    .withPermit(
      Effect.gen(function* () {
        ...
        // Always yield before the heavy synchronous ZIP validation path so the
        // caller receives its operation id immediately and can cancel it.
        const fiber = yield* Effect.forkIn(
          Effect.yieldNow.pipe(Effect.andThen(Effect.interruptible(work))),
          serviceScope,
        );
