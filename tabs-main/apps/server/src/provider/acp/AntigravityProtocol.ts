...
// AG-002: Distinguish native Antigravity interaction questions from security approval requests.
...
// AG-004: Surface native prompt-injection warnings on Allow Always without losing fidelity.
/**
...
// AG-003: Keep Antigravity tool payloads bounded before retaining runtime state.
...
// AG-006: Classify Antigravity subagent calls vs ordinary MCP tools.
...
export type AntigravitySubagentState = "active" | "finished";

/**
 * `start_subagent` completing means the launch request returned; it does not
 * prove that the launched agent finished. The enclosing turn is the only
 * completion boundary exposed by this Antigravity ACP version.
 */
export function planAntigravitySubagentUpdate(input: {
  readonly toolCall: AcpToolCallState;
  readonly rawPayload: unknown;
  readonly previousState: AntigravitySubagentState | undefined;
}): {
  readonly start: boolean;
  readonly progress: boolean;
  readonly completion: "failed" | undefined;
  readonly nextState: AntigravitySubagentState;
} | null {
  if (input.previousState === "finished" || isAntigravitySubagentReplayStart(input.rawPayload)) {
    return null;
  }
  if (input.toolCall.status === "failed") {
    return {
      start: input.previousState === undefined,
      progress: false,
      completion: "failed",
      nextState: "finished",
    };
  ...
  return {
    start: input.previousState === undefined,
    progress: input.previousState !== undefined || input.toolCall.status === "completed",
