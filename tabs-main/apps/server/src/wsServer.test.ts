...
describe("WebSocket Server", () => {
  ...
  async function createTestServer(
    ...
    const baseDir = options.baseDir ?? makeTempDir("tabs-ws-base-");
  ...
  it("serves persisted attachments for URL-encoded paths", async () => {
    const baseDir = makeTempDir("tabs-state-attachments-encoded-");
    const { attachmentsDir } = deriveServerPathsSync(baseDir, undefined);
  ...
  it("serves static index for root path", async () => {
    const baseDir = makeTempDir("tabs-state-static-root-");
    const staticDir = makeTempDir("tabs-static-root-");
  ...
  it("rejects static path traversal attempts", async () => {
    const baseDir = makeTempDir("tabs-state-static-traversal-");
    const staticDir = makeTempDir("tabs-static-traversal-");
  ...
  it("includes bootstrap ids in welcome when cwd project and thread already exist", async () => {
    const baseDir = makeTempDir("tabs-state-bootstrap-existing-");
  ...
  it("responds to server.getConfig", async () => {
    const baseDir = makeTempDir("tabs-state-get-config-");
  ...
  it("bootstraps default keybindings file when missing", async () => {
    const baseDir = makeTempDir("tabs-state-bootstrap-keybindings-");
    const { keybindingsConfigPath: keybindingsPath } = deriveServerPathsSync(baseDir, undefined);
  ...
  it("falls back to defaults and reports malformed keybindings config issues", async () => {
    const baseDir = makeTempDir("tabs-state-malformed-keybindings-");
    const { keybindingsConfigPath: keybindingsPath } = deriveServerPathsSync(baseDir, undefined);
    ensureParentDir(keybindingsPath);
  ...
  it("ignores invalid keybinding entries but keeps valid entries and reports issues", async () => {
    const baseDir = makeTempDir("tabs-state-partial-invalid-keybindings-");
    const { keybindingsConfigPath: keybindingsPath } = deriveServerPathsSync(baseDir, undefined);
    ensureParentDir(keybindingsPath);
    fs.writeFileSync(
      keybindingsPath,
  ...
  it("pushes server.configUpdated issues when keybindings file changes", async () => {
    const baseDir = makeTempDir("tabs-state-keybindings-watch-");
    const { keybindingsConfigPath: keybindingsPath } = deriveServerPathsSync(baseDir, undefined);
    ensureParentDir(keybindingsPath);
    fs.writeFileSync(keybindingsPath, "[]", "utf8");
  ...
  it("reads keybindings from the configured state directory", async () => {
    const baseDir = makeTempDir("tabs-state-keybindings-");
    const { keybindingsConfigPath: keybindingsPath } = deriveServerPathsSync(baseDir, undefined);
    ensureParentDir(keybindingsPath);
    fs.writeFileSync(
      keybindingsPath,
  ...
  it("upserts keybinding rules and updates cached server config", async () => {
    const baseDir = makeTempDir("tabs-state-upsert-keybinding-");
    const { keybindingsConfigPath: keybindingsPath } = deriveServerPathsSync(baseDir, undefined);
    ensureParentDir(keybindingsPath);
    fs.writeFileSync(
      keybindingsPath,
  ...
  it("returns retryable error when requested turn exceeds current checkpoint turn count", async () => {
    ...
    const workspaceRoot = makeTempDir("tabs-ws-diff-project-");
  ...
  it("keeps orchestration domain push behavior for provider runtime events", async () => {
    ...
    const workspaceRoot = makeTempDir("tabs-ws-project-");
    const createdAt = new Date().toISOString();
    const createProjectResponse = await sendRequest(ws, ORCHESTRATION_WS_METHODS.dispatchCommand, {
      type: "project.create",
  ...
  it("routes terminal RPC methods and broadcasts terminal events", async () => {
    const cwd = makeTempDir("tabs-ws-terminal-cwd-");
  ...
  it("catches websocket message handler rejections and keeps the socket usable", async () => {
    ...
    try {
      ...
      const workspace = makeTempDir("tabs-ws-handler-still-usable-");
  ...
  it("supports projects.searchEntries", async () => {
    const workspace = makeTempDir("tabs-ws-workspace-entries-");
  ...
  it("supports projects.writeFile within the workspace root", async () => {
    const workspace = makeTempDir("tabs-ws-write-file-");
  ...
  it("supports projects.readFile within the workspace root", async () => {
    const workspace = makeTempDir("tabs-ws-read-file-");
    fs.mkdirSync(path.join(workspace, "plans"), { recursive: true });
    fs.writeFileSync(path.join(workspace, "plans", "effect-rpc.md"), "# Plan\n\n- step 1\n");
  ...
  it("supports git.status over websocket", async () => {
    ...
    const gitManager: GitManagerShape = {
      ...
      createPullRequest: vi.fn(() => Effect.void as any),
      preparePullRequestThread,
      runStackedAction,
      generateDiffSummary: vi.fn(() => Effect.void as any),
      generateReview: vi.fn(() => Effect.void as any),
      submitFindingFeedback: vi.fn(() => Effect.void as any),
      getReviewHistory: vi.fn(() => Effect.succeed({ records: [] }) as any),
    };
