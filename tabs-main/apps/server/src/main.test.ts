...
import { CliConfig, recordStartupHeartbeat, tabsCli, type CliConfigShape } from "./main";
...
const runCli = (
  ...
  env: Record<string, string> = { TABS_NO_BROWSER: "true" },
) => {
  return Command.runWith(tabsCli, { version: "0.0.0-test" })(args).pipe(
    Effect.provide(
...
it.layer(testLayer)("server CLI command", (it) => {
  ...
  it.effect.skip("parses all CLI flags and wires scoped start/stop", () =>
    Effect.gen(function* () {
      ...
      assert.equal(resolvedConfig?.baseDir, "/tmp/tabs-cli-home");
      assert.equal(resolvedConfig?.stateDir, "/tmp/tabs-cli-home/dev");
  ...
  it.effect.skip("uses env fallbacks when flags are not provided", () =>
    Effect.gen(function* () {
      yield* runCli([], {
        TABS_MODE: "desktop",
        TABS_PORT: "4999",
        TABS_HOST: "100.88.10.4",
        TABS_HOME: "/tmp/tabs-env-home",
        VITE_DEV_SERVER_URL: "http://localhost:5173",
        TABS_NO_BROWSER: "true",
        TABS_AUTH_TOKEN: "env-token",
      });
      ...
      assert.equal(resolvedConfig?.baseDir, "/tmp/tabs-env-home");
      assert.equal(resolvedConfig?.stateDir, "/tmp/tabs-env-home/dev");
  ...
  const openBootstrapFd = Effect.fn(function* (payload: Record<string, unknown>) {
    ...
    const filePath = yield* fs.makeTempFileScoped({ prefix: "tabs-bootstrap-", suffix: ".ndjson" });
  ...
  it.effect.skip("uses bootstrap envelope values as fallbacks when CLI and env are absent", () =>
    Effect.gen(function* () {
      const fd = yield* openBootstrapFd({
        ...
        port: 4888,
        host: "127.0.0.2",
        tabsHome: "/tmp/tabs-bootstrap-home",
      ...
      assert.equal(resolvedConfig?.baseDir, "/tmp/tabs-bootstrap-home");
      assert.equal(resolvedConfig?.stateDir, "/tmp/tabs-bootstrap-home/dev");
      assert.equal(resolvedConfig?.devUrl?.toString(), "http://127.0.0.1:5173/");
      assert.equal(resolvedConfig?.noBrowser, true);
  ...
  it.effect.skip("applies CLI then env precedence over bootstrap envelope values", () =>
    Effect.gen(function* () {
      ...
      yield* runCli(["--port", "4999", "--host", "0.0.0.0", "--auth-token", "cli-token"], {
        ...
        TABS_HOME: "/tmp/tabs-env-home",
        TABS_NO_BROWSER: "true",
        TABS_AUTO_BOOTSTRAP_PROJECT_FROM_CWD: "true",
        TABS_LOG_WS_EVENTS: "true",
      });
  ...
  it.effect.skip("supports CLI and env for bootstrap/log websocket toggles", () =>
    Effect.gen(function* () {
      yield* runCli(["--auto-bootstrap-project-from-cwd"], {
        TABS_MODE: "desktop",
        TABS_LOG_WS_EVENTS: "false",
        TABS_AUTO_BOOTSTRAP_PROJECT_FROM_CWD: "false",
        TABS_NO_BROWSER: "true",
      });
