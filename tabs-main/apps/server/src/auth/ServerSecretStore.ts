...
export const make = Effect.gen(function* () {
  ...
  const readAfterConcurrentCreate = (
    name: string,
    attemptsRemaining = 32,
  ): Effect.Effect<Uint8Array, SecretStoreError> =>
    get(name).pipe(
      Effect.flatMap(
        Option.match({
            ...
            value.byteLength > 0
              ...
              : attemptsRemaining > 0
                ? Effect.yieldNow.pipe(
                    Effect.flatMap(() => readAfterConcurrentCreate(name, attemptsRemaining - 1)),
                  )
                : Effect.fail(new SecretStoreConcurrentReadError({ resource: `secret ${name}` })),
          onNone: () =>
            attemptsRemaining > 0
              ? Effect.yieldNow.pipe(
                  Effect.flatMap(() => readAfterConcurrentCreate(name, attemptsRemaining - 1)),
          ...
          onNone: () =>
            crypto.randomBytes(bytes).pipe(
              ...
              Effect.flatMap((generated) => {
                // Some filesystem implementations transfer the buffer passed to
                // writeAll. Persist a copy so the value returned to the caller
                // can never be detached by the write.
                const persisted = Uint8Array.from(generated);
                return create(name, persisted).pipe(
                  Effect.as(generated),
                  Effect.catchIf(isSecretStoreError, (error) =>
                    isSecretAlreadyExistsError(error)
                      ? readAfterConcurrentCreate(name)
                      : Effect.fail(error),
                  ),
                );
              }),
