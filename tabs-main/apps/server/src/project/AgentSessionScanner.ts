...
import { HostProcessEnvironment, HostProcessPlatform } from "@tabs/shared/hostProcess";
...
>()("tabs/project/AgentSessionScanner") {}
...
function selectMetadataTranscripts(transcripts: ReadonlyArray<TranscriptCandidate>) {
  const byInstance = new Map<ProviderInstanceId, TranscriptCandidate[]>();
  for (const transcript of transcripts) {
    const list = byInstance.get(transcript.providerInstanceId);
    if (list) {
      list.push(transcript);
    } else {
      byInstance.set(transcript.providerInstanceId, [transcript]);
    }
  }

  const selected: Array<TranscriptCandidate> = [];
  let pending = Array.from(byInstance.values(), (entries) => entries.values());
...
function isTabsManagedWorktree(
  ...
  return (
    ...
    normalized.includes("/.tabs/worktrees/") ||
...
export const make = Effect.gen(function* () {
  ...
  const isExcludedProjectPath = (candidatePath: string) =>
    ...
    isTabsManagedWorktree(candidatePath, worktreesDir, foldWorktreeCase);
  ...
  const readTranscript = (
    ...
    recordLimit: number,
    source: AgentSessionSource,
  ): Effect.Effect<{ records: Array<DecodedTranscriptRecord>; recordCount: number } | null> =>
    Effect.gen(function* () {
  ...
  const discoverClaudeTranscripts = (
    homePath: string,
    providerInstanceId: ProviderInstanceId,
    operationBudget: number,
  ): Effect.Effect<{ transcripts: Array<TranscriptCandidate>; truncated: boolean }> =>
    Effect.gen(function* () {
  ...
  const discoverCodexTranscripts = (
    homePath: string,
    providerInstanceId: ProviderInstanceId,
    operationBudget: number,
  ): Effect.Effect<{ transcripts: Array<TranscriptCandidate>; truncated: boolean }> =>
  ...
  const groupTranscriptsByCwd = (
    source: AgentSessionSource,
    transcripts: ReadonlyArray<TranscriptCandidate>,
    budget: MetadataReadBudget,
  ): Effect.Effect<Array<RawCandidate>> =>
    Effect.gen(function* () {
  ...
  const collectCandidates = (): Effect.Effect<
    { candidates: Array<RawCandidate>; truncated: boolean },
    AgentSessionScanError
  > =>
    Effect.gen(function* () {
  ...
  const scan: AgentSessionScanner["Service"]["scan"] = Effect.gen(function* () {
    ...
    const importedProjectsByRoot = new Map<string, (typeof snapshot.projects)[number]>();
  ...
  const prepareRecentThreads = (
    ...
    Effect.gen(function* () {
      ...
      const completedByFile = new Map<string, AgentSessionImportSource[]>();
      for (const source of completedSources) {
        const key = `${source.providerInstanceId}\0${source.filePath}`;
        const list = completedByFile.get(key);
        if (list) {
          list.push(source);
        } else {
          completedByFile.set(key, [source]);
        }
      }
