...
function hasImportBlockingActivity(
  ...
  const linkedPRs = (thread as { linkedPullRequests?: ReadonlyArray<unknown> }).linkedPullRequests;
  return (
    ...
    (linkedPRs != null && linkedPRs.length > 0) ||
  ...
  yield* Stream.runForEach(threads, (outcome) =>
    Effect.gen(function* () {
      ...
      const imported = yield* Effect.gen(function* () {
        ...
        const existingThread: OrchestrationThread | undefined = snapshot.threads.find(
          (t) => t.id === threadId,
        );
        ...
        if (existingThread && existingThread.projectId !== input.projectId) {
          return yield* new AgentSessionThreadProjectConflictError({
            threadId,
            expectedProjectId: input.projectId,
            actualProjectId: existingThread.projectId,
          });
        }

        const importedHistoryPresent = existingThread ? hasImportedHistory(existingThread) : false;
        if (existingThread && importedHistoryPresent && Option.isSome(existingBinding)) {
          yield* directory.recordImportedTranscript({ threadId, source: outcome.source });
          return true;
        }

        if (existingThread && hasImportBlockingActivity(existingThread, importedHistoryPresent)) {
