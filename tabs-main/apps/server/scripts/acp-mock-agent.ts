...
const program = Effect.gen(function* () {
  ...
  yield* agent.handleInitialize((request) =>
    Effect.sync(() => {
      // NOTE: the pinned effect-smol `ndJsonRpc` wire serialization nulls the
      // boolean values inside `_meta` (a `Record<string, unknown>`) in transit,
      // so an enabled `parameterizedModelPicker: true` arrives here as `null`
      // while the key is still present. Detect the capability by key presence
      // rather than an exact `=== true` match. (No caller sends `false`.)
