...
import { ChevronDownIcon, ChevronUpIcon, PlusIcon, RotateCcwIcon, SaveIcon, Trash2Icon } from "lucide-react";
                        ...
                        onClick={() =>
                          tool.kind === "custom_embed"
                            ? setCustomEmbedDrafts((current) => {
                                ...
                                return draftIndex < 0 ? current : reorderItems(current, draftIndex, -1);
                              })
                            : tool.kind === "custom_process"
                              ? setServerProcessDrafts((current) => {
                                  const draftIndex = current.findIndex(
                              ...
                              : upsertProjectSettings(activeProjectId, (current) => {
                                  ...
                                  const persistedIndex = tools.findIndex((entry) => entry.id === tool.id);
                                  const previous = tools[persistedIndex - 1];
                                  const currentTool = tools[persistedIndex];
                                  if (persistedIndex < 0 || !previous || !currentTool) return current;
              ...
              <div className="space-y-3 p-3">
                {customEmbedDrafts.length === 0 ? (
                  <div className="rounded-xl border border-dashed border-border/70 p-4 text-sm text-muted-foreground">
                    No custom tabs configured yet. Add one, edit it, then save it into the toolbar.
                  </div>
                ) : (
                  customEmbedDrafts.map((draft, index) => {
                    const dirty = isCustomEmbedDraftDirty(draft);
                    return (
                      <div key={draft.id} className="space-y-3 rounded-xl border border-border/70 p-3">
                        <div className="flex items-start justify-between gap-3">
                          <div className="min-w-0">
                            <div className="text-sm font-medium text-foreground">
                              {draft.label.trim().length > 0 ? draft.label : "Untitled browser tab"}
                            </div>
                        ...
                        <Input
                          ...
                          onChange={(event) =>
                            setCustomEmbedDrafts((current) =>
                              current.map((entry) =>
                                entry.id === draft.id ? { ...entry, url: event.target.value } : entry,
                              ),
                            )
                          }
                          placeholder="https://www.figma.com/file/..."
                        ...
                        <div className="flex items-center justify-between gap-3">
                          ...
                          <Switch
                            ...
                            onCheckedChange={(checked) =>
                              setCustomEmbedDrafts((current) =>
                                current.map((entry) =>
                                  entry.id === draft.id ? { ...entry, visible: Boolean(checked) } : entry,
                                ),
                              )
                            }
                          />
          ...
          <div className="space-y-3">
            <div className="flex items-start justify-between gap-3">
              <div>
                ...
                <div className="text-xs text-muted-foreground">
                  Add project-specific terminal tools that auto-run predefined commands when the tab
                  opens. Save to add them into the toolbar, then reorder them above if needed.
                </div>
              </div>
              <div className="flex items-center gap-2">
                {serverProcessesDirty ? (
                  <>
                    <Button type="button" size="sm" variant="ghost" onClick={resetServerProcesses}>
                      <RotateCcwIcon className="size-3.5" />
                      Cancel
                    </Button>
            ...
            <ScrollArea className="max-h-[32rem] rounded-xl border border-border/70">
              <div className="space-y-3 p-3">
                {serverProcessDrafts.length === 0 ? (
                  <div className="rounded-xl border border-dashed border-border/70 p-4 text-sm text-muted-foreground">
                    No terminal tabs configured yet. Add one, edit it, then save it into the toolbar.
                  </div>
                ) : (
                      ...
                      <div key={draft.id} className="space-y-3 rounded-xl border border-border/70 p-3">
                        <div className="flex items-start justify-between gap-3">
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            <Button
                              ...
                              disabled={index === 0}
                              onClick={() =>
                                setServerProcessDrafts((current) => reorderItems(current, index, -1))
                              }
                            >
                              <ChevronUpIcon className="size-3.5" />
                            </Button>
                        ...
                        <div className="space-y-2">
                          ...
                          <div className="space-y-2">
                            {draft.commands.map((command, commandIndex) => (
                              <div key={`${draft.id}-step-${commandIndex}`} className="flex gap-2">
                                <Input
                                  value={command}
                                  onChange={(event) =>
                                    setServerProcessDrafts((current) =>
                        ...
                        <Input
                          ...
                          onChange={(event) =>
                            setServerProcessDrafts((current) =>
                              current.map((entry) =>
                                entry.id === draft.id ? { ...entry, cwd: event.target.value } : entry,
                              ),
                            )
                          }
                          placeholder={activeProject.cwd}
