    ...
    <div className="pointer-events-none absolute left-1/2 top-5 z-40 w-[min(38rem,calc(100%-1.5rem))] -translate-x-1/2 px-3">
      <div className="flex items-start gap-3 rounded-2xl border border-border/80 bg-background/95 px-4 py-3 shadow-2xl shadow-black/40 ring-1 ring-white/5 backdrop-blur-xl">
        ...
        <div className="min-w-0">
          <div className="text-sm font-semibold text-foreground">Preview hidden for screen sizing</div>
...
function DesktopCodeTool(props: { project: Project }) {
      ...
      <div className="flex h-full min-h-0 min-w-0 flex-col bg-background">
        <div className="border-b border-border/70 bg-card/60 px-4 py-3">
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div className="min-w-0">
              <div className="text-sm font-medium text-foreground">Embedded Code-OSS unavailable</div>
  ...
  return (
    <div className="relative flex h-full min-h-0 min-w-0 flex-col bg-background">
      <div
        ref={hostRef}
        className="absolute inset-0 min-h-0 min-w-0 bg-background"
      />
              ...
              <div className="grid min-h-0 flex-1 gap-4 2xl:grid-cols-[minmax(0,1.1fr)_minmax(0,0.9fr)]">
                <div className="flex min-h-0 flex-col gap-3 rounded-xl border border-border/70 bg-background/40 p-4">
                  ...
                  <div className="space-y-1">
                    <div className="flex flex-wrap items-center gap-2">
                      <div className="text-sm font-medium text-foreground">
                        {resolverMode === "manual" ? "Manual conflict resolver" : "AI conflict fix"}
                      </div>
                      {resolverMode === "manual" ? (
      ...
      <div className="grid min-h-0 min-w-0 gap-4 xl:grid-rows-[auto_auto_minmax(0,1fr)]">
        <Card className="min-h-0 min-w-0 overflow-hidden">
          <CardHeader className="min-w-0 border-b border-border/60 pb-4">
            ...
            <CardDescription className="break-words">{branchesSection.description}</CardDescription>
          </CardHeader>
                  ...
                  branchesQuery.data.branches.map((branch: GitBranch) => (
                    ...
                    >
                      <div className="flex items-start justify-between gap-2">
                        ...
                        >
                          <div className="truncate font-medium text-foreground">{branch.name}</div>
                          <div className="mt-1 text-xs text-muted-foreground">
                            {branch.isRemote
                              ? branch.remoteName
                  ...
                  <div className="space-y-3 p-3">
                    ...
                    <div className="flex gap-2">
                      ...
                      <Button
                        ...
                        size="sm"
                        variant="outline"
                        disabled={!branchToolsBranch || !selectedUpstreamBranch || branchToolsBusy}
                    ...
                    <Button
                      ...
                      disabled={!branchToolsBranch || branchToolsBranch.current || branchToolsBusy}
                  ...
                  <div className="space-y-3 p-3">
                    {currentOperation ? (
                      <div className="rounded-lg border border-border/70 bg-background/50 p-3 text-sm text-muted-foreground">
                        The active {currentOperation.kind} is already surfaced in the main workspace above.
                      </div>
                    ) : (
                      <>
                        <Select
                          ...
                          <SelectPopup className="max-h-80">
                            <div className="px-2 py-2">
                              <Input
                                ...
                                onChange={(event) => setOperationBranchSearch(event.target.value)}
                                onKeyDown={(event) => event.stopPropagation()}
        ...
        <Card className="min-h-0 min-w-0">
          <CardHeader className="min-w-0 border-b border-border/60 pb-4">
            ...
            <CardDescription className="break-words">{historySection.description}</CardDescription>
          </CardHeader>
          <CardContent className="grid min-h-0 gap-4 pt-6">
                  ...
                  historyCommits.map((commit, index) => (
                    <button
                      ...
                      className={cn(
                        ...
                        activeHistoryCommit?.sha === commit.sha && !selectedPath && "bg-primary/10",
                      )}
                    >
            ...
            <Collapsible open={stashPanelOpen} onOpenChange={setStashPanelOpen}>
              <div className="rounded-xl border border-border/70 bg-background/60">
                <CollapsibleTrigger className="flex w-full min-w-0 items-center justify-between gap-3 px-3 py-3 text-left">
                  <div className="min-w-0">
                    <div className="text-sm font-medium text-foreground">{stashesSection.title}</div>
                    <div className="mt-1 break-words text-xs text-muted-foreground">
...
function DesktopBrowserTool(props: { project: Project; projectSettings: ProjectWorkspaceSettings }) {
  const api = readNativeApi();
  const bridge = window.desktopBridge;
  ...
  useEffect(() => {
    ...
    if (viewportSelectorOpen) {
      ...
      void bridge.activateBrowserSession({
        projectId: props.project.id,
      }).catch(() => undefined);
    }
          ...
          <div className="flex flex-wrap items-center gap-1">
            ...
            <div className="flex min-w-[12rem] flex-1 items-center gap-1.5">
              ...
              <Button
                type="button"
                size="xs"
                onClick={() => setBrowserCurrentUrl(props.project.id, normalizeBrowserUrl(draftUrl))}
              >
                Go
              </Button>
            </div>
...
function EmbeddedBrowserTool(props: { project: Project; projectSettings: ProjectWorkspaceSettings }) {
  const api = readNativeApi();
  const browserState = useWorkspaceShellStore((state) => {
    const existing = state.browserStateByProjectId[props.project.id];
    if (existing) {
...
function CustomProcessTool(props: {
  ...
  const commands = useMemo(
    () => props.process.commands.map((command) => command.trim()).filter((command) => command.length > 0),
        ...
        <div className="flex h-full items-center justify-center p-6">
          <Card className="max-w-lg">
            <CardHeader>
              ...
              <CardDescription>
                Running {commands.length === 1 ? "the configured command" : "the configured commands"} in the embedded terminal.
              </CardDescription>
            </CardHeader>
          </Card>
        </div>
...
export function WorkspaceShell(props: { agentsContent: ReactNode; settingsContent: ReactNode }) {
  ...
  useEffect(() => {
    ...
    const existingProject = projects.find((project) => project.cwd === embeddedMode.workspaceRoot) ?? null;
    if (existingProject) {
